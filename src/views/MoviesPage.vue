  ...
  <div class="main-content" :class="appStore.sidebarCollapsed ? 'collapsed' : 'expanded'">
    ...
    <div class="featured-banner">
      <div class="featured-content">
        ...
        <p class="featured-description">Paul Atreides unites with Chani and the Fremen while seeking revenge against the conspirators who destroyed his family.</p>
...
type RailId = 'trending' | 'newReleases' | 'action' | 'comedy' | 'drama' | 'sciFi' | 'horror' | 'romance' | 'thriller' | 'animation' | 'documentary' | 'international'
type Rail = {
  id: RailId
  title: string
  icon: string
...
const buildRail = (id: RailId, title: string, icon: string, movies: Movie[]): Rail => ({
  id,
  title,
  icon,
  movies,
...
const rails = ref<Rail[]>([
  buildRail('trending', '🔥 Trending Now', 'bi-fire', allMovies.slice(0, 12)),
  buildRail('newReleases', '✨ New Releases', 'bi-stars', allMovies.filter(m => m.year >= 2023)),
  buildRail('action', '💥 Action & Adventure', 'bi-lightning-charge-fill', allMovies.filter(m => m.id >= 200 && m.id < 300)),
  buildRail('comedy', '😂 Comedy Picks', 'bi-emoji-laughing', allMovies.filter(m => m.id >= 300 && m.id < 400)),
  buildRail('drama', '🎭 Award-Winning Drama', 'bi-award-fill', allMovies.filter(m => m.id >= 400 && m.id < 500)),
  buildRail('sciFi', '🚀 Sci-Fi & Fantasy', 'bi-stars', allMovies.filter(m => m.id >= 500 && m.id < 600)),
  buildRail('horror', '👻 Horror & Thrillers', 'bi-emoji-dizzy', allMovies.filter(m => m.id >= 600 && m.id < 700)),
  buildRail('romance', '💕 Romance & Drama', 'bi-heart-fill', allMovies.filter(m => m.id >= 700 && m.id < 800)),
  buildRail('thriller', '🔪 Mystery & Suspense', 'bi-eye-fill', allMovies.filter(m => m.id >= 800 && m.id < 900)),
  buildRail('animation', '🎨 Animation & Family', 'bi-palette-fill', allMovies.filter(m => m.id >= 900 && m.id < 1000)),
  buildRail('documentary', '📽️ Documentaries', 'bi-film', allMovies.filter(m => m.id >= 1000 && m.id < 1100)),
  buildRail('international', '🌍 International Cinema', 'bi-globe', allMovies.filter(m => m.id >= 1100)),
])
